...
function FastTravel:ListDestination(traveller)
	-- Print("ListDestination, reload is bad...")
	local x, y, z = self.inst.Transform:GetWorldPosition()
	local dests = TheSim:FindEntities(x, y, z, find_dist, FTSignTag)
	local dest = {}

	for k, v in pairs(dests) do
		if v ~= self.inst and v.components.fasttravel and
			not
			(
			v.components.fasttravel.ownership and v:HasTag(ownershiptag) and traveller.userid ~= nil and
				not v:HasTag('uid_' .. traveller.userid)) then
			if v.components.writeable and v.components.writeable:GetText() then
				table.insert(dest, v)
			end
		end
	end
	...
	table.sort(
		...
		function(info1, info2)
			local nameInfo1 = info1.components.writeable:GetText()
			local nameInfo2 = info2.components.writeable:GetText()
			if nameInfo1 == "" then
				return false
			end
			if nameInfo2 == "" then
			...
			return string.lower(nameInfo1) < string.lower(nameInfo2)
		end
	)

	-- 说实话我本来觉得很无语，就连照抄人家的代码都能出bug，这也太夸张了吧，真的程序是永远不会让我失望的，但现在我却发现这或许还真的就是不如我理所当然那么想的，并不是拼接了人家的代码就能运作，惊了。

	self.destinations = dest
	self.site = self.site or 1 -- 这里？为什么第一次写就没有用到下面的+1，所以不能是#dest而必须是1，很诡异。
	self.totalsites = #dest
end

function FastTravel:SelectDestination(traveller)
	-- Print("SelectDestination")
	-- Print(self.site)
	if traveller == nil then return end
	-- Print(self.destinations == nil)
	if next(self.destinations) == nil then -- 可能会出现意想不到的那种情况吧，比如说传送过程中敲掉什么的，但从我游玩的体验来说一切都很小心，虽然写mod的立场要做到很完善。
		self:ListDestination(traveller)
	end
	local comment = self.inst.components.talker
	local talk = traveller.components.talker

	if self.ownership and self.inst:HasTag(ownershiptag) and traveller.userid ~= nil and
		not self.inst:HasTag('uid_' .. traveller.userid) then
	...
	if destination and destination.components.fasttravel then
		...
		self.traveltask = self.inst:DoTaskInTime(8, function()
			...
			for k, who in pairs(travellers) do
				...
				elseif destination.components.fasttravel.ownership and destination:HasTag(ownershiptag) and who.userid ~= nil and
				...
				elseif who.components.hunger and who.components.hunger.current >= cost_hunger and who.components.sanity and
					who.components.sanity.current >= cost_sanity then
		...
		self.traveltask1 = self.inst:DoTaskInTime(7, function() comment:Say("Travel in 1 second.", 1)
			self.inst.SoundEmitter:PlaySound("dontstarve/HUD/craft_down")
			-- added for alphabetical mode
			-- Print("为什么我就是忘记去写debug的东西?????")
			-- print(self.site)
			self.site = #self.destinations
			self.destinations = {}
		end)
